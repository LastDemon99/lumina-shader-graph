...
import { Plus, Wand2, Trash2, Download, Upload, ZoomIn, ZoomOut, MousePointer2, Box, Square, Save, Search, Layers, Network, CheckCircle2, Loader2, AlertTriangle, Sparkles } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const textureUniforms = useMemo(() => {
    ...
    nodes.filter(n => ['texture', 'sampleTexture2DLOD', 'gatherTexture2D', 'sampleTexture2DArray'].includes(n.type)).forEach(n => {
        // Determine Asset URL
    ...
    nodes.filter(n => n.type === 'textureAsset' || n.type === 'texture2DArrayAsset').forEach(n => {
        if (n.data.textureAsset) {
            ...
            if (!map[uniformName]) {
                // Default settings for raw asset preview
  ...
  // Wrap in useCallback so it catches latest state
  const saveGraph = useCallback(async (saveAsNew = false) => {
      ...
      if (!saveAsNew && fileHandle) {
          try {
              ...
              alert("Failed to overwrite file. Try 'Save As'.");
          }
      } else {
          if ('showSaveFilePicker' in window) {
              try {
                  ...
                  await writable.close();
                  setFileHandle(handle);
                  setFileName(handle.name.replace('.json', ''));
              } catch (err) {
                  if ((err as Error).name !== 'AbortError') console.error("Save As Error:", err);
              }
          } else {
              // Fallback Download
              ...
              a.href = url;
              a.download = `${fileName || `shader-graph-${Date.now()}`}.json`;
              a.click();
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Allow shortcuts if not typing in input
  ...
  }, [selectedNodeIds, activeTab, nodes, connections, previewMode, fileHandle, fileName, saveGraph]);
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    // Left Mouse Button (Button 0) on Background -> Start Selection Box
    if (e.button === 0 && !connecting) {
        if (e.target === canvasRef.current || (e.target as HTMLElement).tagName === 'svg') {
            if (!e.ctrlKey) {
                setSelectedNodeIds(new Set()); // Clear selection if not holding Ctrl
            }
            
            // Calculate relative position for the selection div overlay
            const rect = canvasRef.current?.getBoundingClientRect();
            if(rect) {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    // Update generic mouse pos for drawing connection lines
  ...
  const handleMouseUp = (e: React.MouseEvent) => {
    ...
    if (selectionBox) {
        ...
        if (rect && canvasRef.current) {
            // Calculate Box in Graph Coordinates
            const startX_screen = selectionBox.startX;
            const startY_screen = selectionBox.startY;
            const endX_screen = selectionBox.currentX;
            const endY_screen = selectionBox.currentY;

            // Normalize rectangle
            const left = Math.min(startX_screen, endX_screen);
            const top = Math.min(startY_screen, endY_screen);
            const width = Math.abs(endX_screen - startX_screen);
            const height = Math.abs(endY_screen - startY_screen);

            // Convert selection box to Graph Space for intersection checking
            ...
            nodes.forEach(node => {
                ...
                const nodeScreenW = nodeWidth * viewport.zoom;
                const nodeScreenH = nodeHeight * viewport.zoom;

                // Check Intersection
                if (left < nodeScreenX + nodeScreenW &&
                    left + width > nodeScreenX &&
                    top < nodeScreenY + nodeScreenH &&
                    top + height > nodeScreenY) {
  ...
  const handleNodeMouseDown = (e: React.MouseEvent, id: string) => {
    ...
    // Regular Click
        ...
        // If clicking a node that is NOT already selected, clear others and select this one
        if (!selectedNodeIds.has(id)) {
            setSelectedNodeIds(new Set([id]));
        }
        // If clicking a node that IS selected, we keep the group selection for dragging
    }
  ...
  // Helper to check if types are broadly compatible
  const isTypeCompatible = (sourceType: SocketType, targetType: SocketType) => {
      // Strict check for textureArray
  ...
  const handleSocketMouseUp = (e: React.MouseEvent, nodeId: string, socketId: string, isInput: boolean, type: SocketType) => {
    e.stopPropagation();
    if (connecting) {
      if (connecting.nodeId === nodeId) return; // Self connection
      if (connecting.isInput === isInput) return; // In-In or Out-Out
      ...
      // Atomic update to prevent race conditions when replacing connections
      setConnections(prev => {
        // Remove existing connection on this specific target socket (Inputs only support 1 connection)
  ...
  const updateNodeData = (id: string, data: any) => {
    setNodes(prev => prev.map(n => {
        if (n.id === id) {
            ...
            // Special handling for Swizzle node mask updates to change output socket type
  ...
  const addNode = (type: NodeType) => {
    ...
    // Set meaningful defaults for Remap node to be useful immediately
    if (type === 'remap') {
        newNode.data.inputValues = {
            inMinMax: { x: -1, y: 1 },
            outMinMax: { x: 0, y: 1 }
        };
    }
  ...
  const getSocketPos = (nodeId: string, socketId: string, isInput: boolean) => {
    ...
    // Use DOM positioning for pixel-perfect alignment
    if (socketEl && nodeEl) {
      const sRect = socketEl.getBoundingClientRect();
      const nRect = nodeEl.getBoundingClientRect();
      
      // Calculate relative position of socket center vs node top-left
      // We perform this calculation in screen space, then divide by zoom to get local units
    ...
    // Fallback if DOM is not ready (mostly for initial render)
    const width = 160;
    const xOffset = 9;
    // Rough estimate header (28) + padding (8) + average 1 socket height (30)
    return {
      x: node.x + (isInput ? -xOffset : width + xOffset), 
      y: node.y + 50 
    };
  };
  ...
  const handleGeminiGenerate = async () => {
    ...
    // Explicitly cast prompt to string to avoid unknown type issues
    ...
    <div className="w-screen h-screen bg-[#111] overflow-hidden flex flex-col relative" onContextMenu={e => e.preventDefault()}>
      ...
      <div className="h-12 bg-[#1e1e1e] border-b border-gray-700 flex items-center justify-between px-4 z-50 shrink-0 relative z-20">
          <div className="flex items-center gap-2">
            ...
            {fileHandle && <span className="text-xs text-gray-500 font-mono flex items-center bg-black px-2 py-0.5 rounded border border-gray-800">{fileName}.json</span>}
          ...
          <div className="flex items-center gap-2">
               ...
               <button onClick={handleOpen} className="bg-gray-800 hover:bg-gray-700 p-1.5 rounded text-gray-300 cursor-pointer transition-colors" title="Open (Ctrl+O)">
                 <Upload className="w-4 h-4" /> 
               </button>
               {/* Hidden fallback input for non-FSA browsers */}
      ...
      <div className={`w-full h-full absolute inset-0 flex flex-col ${activeTab === 'graph' ? 'z-10' : 'z-0 invisible'}`}>
        ...
        {/* LAYER 1: Global Canvas (Shader Previews) - Z=15 (Above Nodes, Below UI) */}
        <GlobalCanvas />

        {/* LAYER 2: Sidebar (z-20) */}
        ...
        <div className={`absolute top-4 right-4 w-72 z-20 pointer-events-auto flex flex-col gap-2 ${activeTab === 'graph' ? 'opacity-100' : 'opacity-0'}`}>
          <div className="w-full h-72 bg-black rounded-lg overflow-hidden border border-gray-700 shadow-xl relative group">
             ...
             {/* Use SceneView here to ensure it's visible on top of the Sidebar Background */}
        ...
        {/* LAYER 3: Graph Interaction Area (Nodes) - Z=10 */}
        <div 
          ref={canvasRef}
      ...
      <div className={`w-full h-full absolute inset-0 bg-[#0a0a0a] ${activeTab === 'scene' ? 'z-10' : 'z-0 invisible pointer-events-none opacity-0'}`}>
